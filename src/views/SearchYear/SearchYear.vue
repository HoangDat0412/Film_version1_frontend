...
watchEffect(async () => {
    ...
    await film.searchYear({
      year: route.params.name,
...
const handlepage = (page) => {
    router.push({ path: `/year/${route.params.name}/${page}` })
}
const handleback = ()=>{
...
<template>
    <main>
        <div>
            ...
            <nav aria-label="Page navigation example" v-if="film.pagination > 1">
                <ul class="pagination">
                    <li class="page-item">
                    ...
                    <li class="page-item" v-for="i in film?.pagination" :key="i"><a @click="()=>handlepage(i)" class="page-link" >{{ i
                            }}</a></li>
                    <li class="page-item">
